"""
Inference Module — InsiderTransformerAE
========================================
InferenceRunner class for scoring users with the trained model.

Pipeline:
1. Load model, preprocessing artifacts, and threshold
2. Session-aware feature engineering (DuckDB SQL)
3. 60-day sequence creation with zero-padding
4. Anomaly scoring using reconstruction error
5. Per-user risk assessment and report generation

Used by scripts/06_inference.py as a thin CLI wrapper.
"""
...
def load_threshold(output_dir: Path, method: str) -> float:
    """
    ...
    computed from calibration scores.
...
def build_user_report(user_id, user_scores, user_dates, threshold,
    ...
    Returns:
        dict: User report entry with severity, scores, and risk indicators
    """
...
class InferenceRunner:
    """
    InferenceRunner for InsiderTransformerAE.

    Manages the full inference lifecycle: model loading, feature engineering,
    sequence creation, scoring, and report generation.

    Args:
        config: Configuration dictionary (from config.yaml)
        user_id: Score a specific user (None = all users)
        start_date: Start date for scoring window (YYYY-MM-DD)
        end_date: End date for scoring window (YYYY-MM-DD)
        threshold: Override anomaly threshold (None = load from eval)
        threshold_method: Threshold method to load if threshold is None
        top_k: Show top K riskiest users in console output
    ...
    def __init__(
        ...
        self.data_dir = self.project_root / config['data']['processed_dir']
        self.output_dir = self.project_root / 'outputs'
        self.db_path = self.project_root / config['data']['duckdb_path']
    ...
    def run(self):
        ...
        model, n_continuous, _ckpt = load_model(self.output_dir, device, self.config)
        artifacts = load_artifacts(self.data_dir)

        if self.threshold_override is not None:
            threshold = self.threshold_override
            print(f"  Threshold (manual): {threshold:.6f}")
        else:
            threshold = load_threshold(self.output_dir, self.threshold_method)
            print(f"  Threshold ({self.threshold_method}): {threshold:.6f}")

        # --------------------------------------------------------------
        # [2] Feature engineering
        # --------------------------------------------------------------
        user_filter = [self.user_id] if self.user_id else None
        date_range = None
        ...
        filter_parts = []
        if self.user_id:
            filter_parts.append(f"user '{self.user_id}'")
        if date_range:
        ...
        result = run_inference_feature_engineering(
            self.db_path, self.config, artifacts,
        ...
        # [3] Build sequences
        # --------------------------------------------------------------
        lookback = self.config['model']['lookback']
        ...
        for uid in unique_users:
            ...
            report = build_user_report(
                ...
                model, u_cont, u_cat, self.config, device,
            )
        ...
        top_k = min(self.top_k, len(user_reports))
        ...
        for i, r in enumerate(user_reports[:top_k]):
            print(f"  {i+1:<6} {r['user_id']:<25} {r['severity']:<10} "
                  f"{r['anomaly_score_max']:<12.6f} "
                  f"{r['flagged_sequences']}/{r['total_sequences']}")
        ...
        report_output = {
            'metadata': {
                ...
                'threshold_method': self.threshold_method if self.threshold_override is None else 'manual',
        ...
        output_path = (Path(self.output_path_override) if self.output_path_override
                       else self.output_dir / 'inference_report.json')
